...
## 2026-05-31 — pending — 工地專屬背景 + 工程車 2.5D 遊走

- 🎨 **工地專屬背景**：工程車模式從沿用「小鎮」背景換成專屬「工地」場景 —— 半完工的童話城堡鷹架、塔式吊車吊著磚塊、沙堆、粉白施工圍籬、三角錐、開闊泥土前景（portrait `bg-machine.jpg` + landscape `bg-machine-wide.jpg`，ChatGPT 生成、優化 progressive JPEG）
- ✨ **2.5D 遊走視角**：機具不再只是直線橫越，改為從一側進場 → 經過 3 個內部 waypoint（x 逐步橫向推進、y 在工作帶內上下遊走、各帶 ±40px jitter）→ 從另一側離場，像真的在工地裡走動
- 🎨 **景深縮放**：越靠畫面下方＝越近＝越大（`depthScale` 0.72→1.14 依 y 位置線性內插），製造前後縱深；遊走動畫時長 ×1.6、`ease-in-out`，比直線車流更慢更悠閒
- 📦 service-worker 升至 **v53**，APP_ASSETS 納入 2 張工地背景

## 2026-05-31 — 63384b9 — 天燈改 3D sprite + 新增工程車模式（10 機具）
